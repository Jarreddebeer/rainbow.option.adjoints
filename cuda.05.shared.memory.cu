...
static __global__ void adjoint_method_correlation_GPU(
    ...
    __shared__ float shared_mem[28+12*BLOCKSIZE];
...
if (tid < num_sims) {
    ...
    float* __restrict__ Z_indp = &shared_mem[28+6*threadIdx.x];
    float* __restrict__ x23    = &shared_mem[28+6*BLOCKSIZE+3*threadIdx.x];
    float* __restrict__ x24    = &shared_mem[28+9*BLOCKSIZE+3*threadIdx.x];
    ...
    float x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19, x20, x21, x22;
    ...
    winning_asset = 0;
    ST_max = ST_del = ST_veg = 0.0f;

    for (int a=0; a<num_assets; a++) {
        x23[a] = assets[1*num_assets+a]; // vega  
        x24[a] = assets[0*num_assets+a]; // asset
        // init sensitivity vector
        _x_1[a] = 1.0f; _x_2[a] = 0.0f; _x_2v[a] = 1.0f; 
     }

     for (int t=0; t<num_steps; t++)
     {
           // generate correlated rngs
//         for (int a=0; a<num_assets; a++) { reinterpret_cast<float2*>(Z_indp)[2*a] = curand_normal2(&rng_state); Z_corr[a] = 0.0f; }
         for (int a=0; a<num_assets; a++) { reinterpret_cast<float2*>(&Z_indp[2*a])[0] = curand_normal2(&rng_state); Z_corr[a] = 0.0f; }
         for (int row=0; row<num_assets; row++) for (int c=0; c<num_assets; c++) { Z_corr[row] += chlsky[num_assets*row+c] * Z_indp[2*c]; }
         ...
         for (int a=0; a<num_assets; a++)
         {
            ...
            Zs = Z_corr[a];
            Zv = x_8 * Zs + sqrt(1-x_8*x_8) * Z_indp[2*a+1];
//            Zv = x_8 * Zs + sqrt(1-x_8*x_8) * 1.5;
            ...
            // volatility adjoints
            _x23 = 1.0f;
            _x21 = (x23[a] > 0.0f) ? _x23 : 0.0f;
            _x13 = _x16 = _x17 = _x21;
            _x11 = Zv * _x16;
            _x8 = -1 * _x17;
            _x5v = x_5 * _x11;
            _x0v = (0.5f * _x5v / x5) + _x8*x_7;
